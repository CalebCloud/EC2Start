...
- Click on API Gateway under the Lambda triggers, underneath find the gateway you just created and click on details. Under details, copy the API endpoint. 
...
Taking it Further
- This can be taken further by integrating Cognito authorization so that only logged in or authorized users from the user pool will be able to invoke the API and turn on the server.
